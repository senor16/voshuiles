<?php
ob_start();
echo '<h2><a class="black-link" class="settings" href="'.ROOT_URL.'settings">' . $title.'</a>';
if ($action != 'main') {
    echo ' / ' . $action;
}
echo '</h2>';
        ...
        case "profil":
            ...
            <form action="" method="post">
                ...
                <input id="first_name" type="text" required name="first_name"
                       value="<?= $fields['first_name'] ?? $auth->prenom ?>"
                       placeholder="Ex: Michée"
                       class="<?= isset($message['first_name']) ? 'field-error' : '' ?>">
                <br>
                ...
                <?= $message['tel'] ?? '' ?>
                <input id="tel" type="tel" required name="tel" value="<?= $fields['tel'] ?? $auth->tel ?>"
                ...
                <?= $message['town'] ?? '' ?>
                <input id="town" type="text" required name="town" value="<?= $fields['town'] ?? $auth->ville ?>"
                       placeholder="Ex: Maroua"
                       class="<?= isset($message['town']) ? 'field-error' : '' ?>"><br>
                <input class="del-add" type="reset" required  value="Restaurer">
